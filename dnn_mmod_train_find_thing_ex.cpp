...
/*
    ...
    Modified from original source here:
     ...
     https://github.com/davisking/dlib/blob/master/examples/dnn_mmod_train_find_cars_ex.cpp
    ...
    a detector that finds eyes (without requiring face detection).  I will also discuss some
...
int main(int argc, char** argv) try
{
    if (argc != 2)
    {
        ...
        cout << "by typing: " << endl;
        cout << "   ./dnn_mmod_train_find_things_ex dlib_rear_end_vehicles" << endl;
        cout << endl;
    ...
    // image is implicitly assumed to be not a thing, and the algorithm will use it as
    // negative training data.  So every thing must be labeled, either with a normal
    ...
    // 
    // In our present case, many images contain very tiny things in the distance, ones that
    ...
    // only really small and blurry things appear at those sizes.  We will also mark boxes
    ...
    // and ask "is there a thing here?".  If there really is a thing in a specific location in
    // an image then usually many slightly different sliding window locations will produce
    // high detection scores, indicating that there is a thing at those locations.  If we
    // just stopped there then each thing would produce multiple detections.  But that isn't
    // what we want.  We want each thing to produce just one detection.  So it's common for
    ...
    // If non-max suppression just kills everything in a really wide area around a thing then
...
/*
        for (auto& bb : v)
        {
            ...
            // in those cases?  Do you thinge about these boxes?  Are they labeling errors?
            ...
            // should thingefully consider it and not take your dataset as a given.  The work
 ...
 */
    ...
    mmod_options options(boxes_train, 60, 40);
    ...
    const string sync_filename = "mmod_things_sync";
    ...
    while(trainer.get_learning_rate() >= 1e-4)
    {
        ...
        {
            cropper(80, images_train, boxes_train, mini_batch_samples, mini_batch_labels);
...
// #if !defined(DLIB_NO_GUI_SUPPORT)                
//            image_window win;
//            for(std::size_t i = 0; i < mini_batch_samples.size(); i++)
//            {
//                win.clear_overlay();
//                win.set_image(mini_batch_samples[i]);
//                for(auto &r : mini_batch_labels[i])
//                {
//                    win.add_overlay(r);
//                    std::cout << "rect: " << r.rect << " label: " << r.label << " conf: " << r.detection_confidence << " ignore: " << int(r.ignore) << std::endl;
//                }
...
// #endif
        ...
        {
            cropper(80, images_test, boxes_test, mini_batch_samples, mini_batch_labels);
            // We can also randomly jitter the colors and that often helps a detector
            // generalize better to new images.
            for (auto&& img : mini_batch_samples)
            {
    ...
    // Upsampling the data will allow the detector to find smaller things.  Recall that 
    ...
    /*
        ...
        onto the end of it (see dnn_mmod_find_things_ex.cpp for an example of its use).  This
